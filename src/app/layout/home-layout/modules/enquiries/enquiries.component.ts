...
export class EnquiriesComponent {
  ...
  filterCount(): number {
    ...
    if (this.filterValues['type']?.length) count++;
    ...
    this.dialog
      ...
      .subscribe((result: Record<string, any> | undefined) => {
        ...
        this.filterOption.status = this.filterValues['status']?.length
          ...
          : null;
        this.filterOption.type = this.filterValues['type']?.length
          ? this.filterValues['type'].join(',')
          : null;
  ...
  get permissions(): string[] { return this.access.actions('enquiries'); }
  checkPermission() {
