...
// Matches ORDER_EXPORT_ROW_LIMIT in
// elexify-backend/src/services/orderService/exportOrders.js
...
@Component({
  ...
  template: `
    <div class="p-6 sm:p-8">
      ...
      <p class="mt-2 text-sm text-gray-600">Download an Excel (.xlsx) file of the orders below.</p>
      ...
      <div class="mt-5 space-y-3">
        <label class="flex cursor-pointer items-start gap-3 rounded-lg border border-gray-300 p-3 has-[:checked]:border-primary has-[:checked]:bg-primary/5">
          ...
          <span>
            <span class="block text-sm font-semibold text-gray-800">
              Export all orders matching the current filters
            </span>
            <span class="block text-xs text-gray-500 mt-0.5">
              {{ data.filterCount ? data.filterCount + ' filter(s) applied' : 'No filters applied — exports every order' }}{{ data.searchKey ? ' · search "' + data.searchKey + '"' : '' }}
            </span>
          </span>
        </label>
        ...
        >
          ...
          <span>
            ...
            <span class="block text-xs text-gray-500 mt-0.5">
              {{ data.selectionCount ? data.selectionCount + ' order(s) selected on this page' : 'Select rows in the table first (checkboxes) to enable this' }}
            </span>
          </span>
        </label>
      </div>
      ...
      <p *ngIf="scope === 'filtered'" class="mt-4 text-xs text-gray-500">
        Limited to {{ rowLimit }} orders per export — narrow the filters (e.g. by date range) if you have more.
      </p>
...
export class ExportOrdersDialogComponent {
  rowLimit = ROW_LIMIT;
  scope: 'filtered' | 'selected';

  constructor(
    public dialogRef: MatDialogRef<ExportOrdersDialogComponent>,
