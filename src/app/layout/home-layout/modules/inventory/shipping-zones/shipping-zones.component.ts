...
export class ShippingZonesComponent {
  Global = Global;
  item_list: any = [];
  ...
  ngOnInit(): void {
    ...
    this.helperService.actionButtonClick$
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => this.router.navigateByUrl('/inventory/shipping-zones/add'));
  ...
  editItem(item: any) {
    this.router.navigateByUrl(`/inventory/shipping-zones/update/${item._id}`);
  }
  fetchZones() {
  ...
  deleteItem(item: any) {
    this.inventoryService.deleteShippingZone({ _id: item._id }).subscribe({
      next: (res: any) => {
  ...
  get permissions(): string[] { return this.access.actions('shipping_zones'); }
}
